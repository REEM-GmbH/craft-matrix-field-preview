			...
			if (Craft::$app->plugins->isPluginInstalled('matrixmate')) {
				...
				if ($entry) {
					...
					foreach ($entryTypes as $entryType) {
						if (isset($entryType['handle']) && $entryType['uid'] === $blockType->uid) {
							$fieldUid2blockTypeUid = $entryType['uid'];
							$fieldHandle2blockTypeHandle = $entryType['handle'];
							break;
						}
					}
					...
					foreach ($fieldConfig as $key => $value) {
						...
						if ($sectionKey === $sectionHandle) {
							foreach ($value['hiddenTypes'] as $hiddenType) {
								if ($hiddenType === $blockType->handle
								    || ($blockType->uid === $fieldUid2blockTypeUid && $fieldHandle2blockTypeHandle === $hiddenType)
								) {
									continue 3; // Überspringt die äußere Schleife
								}
							}
						}
					}
